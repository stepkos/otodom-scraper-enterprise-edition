...
def check_is_not_unexpected_values(
    apartment_dict: dict, unexpected_values: dict, expected_values: dict
) -> bool:
    for key, value in apartment_dict.items():
...
def cast_categories_to_codes(apartment_dict: dict) -> dict:
    apartment_dict["address_estate"] = address_estate_to_codes[
        apartment_dict["address_estate"]
    ]
    apartment_dict["market"] = market_to_codes[apartment_dict["market"]]
    apartment_dict["finishing_condition"] = finishing_condition_to_codes[
...
def one_hot_encode(apartment_dict: dict) -> dict:
    ...
    for i in range(max(finishing_condition_to_codes.values()) + 1):
        apartment_dict[f"finishing_condition_{i}"] = (
            0 if i != apartment_dict["finishing_condition"] else 1
        )

    for i in range(max(address_estate_to_codes.values()) + 1):
        apartment_dict[f"address_estate_{i}"] = (
            0 if i != apartment_dict["address_estate"] else 1
        )
...
def processing_apartment(apartment: Apartment) -> dict | None:
    ...
    apartment_dict["is_elevator"] = apartment_dict["is_elevator"] or random.choice(
        [False, True]
    )
    apartment_dict["market"] = apartment_dict["market"] or random.choice(
        ["pierwotny", "wtórny"]
    )
    apartment_dict["finishing_condition"] = apartment_dict[
        "finishing_condition"
    ] or random.choice(["do wykończenia", "do zamieszkania", "do remontu"])

    if not all(
        [
            check_is_not_none(apartment_dict),
            check_is_not_unexpected_values(
                apartment_dict, unexpected_values_dict, expected_values_dict
            ),
        ]
    ):
        return None
